    ...
    if ('POST' == this.method) {
      try {
        if (this.isBinary) {
          ...
          xhr.setRequestHeader('Content-type', 'test/plain;charset=UTF-8');
        }
      } catch (e) {}
    }
...
/**
 * Opens the socket (triggers polling). We write a PING message.js to determine
...
Polling.prototype.onData = function(data){
  ...
  var callback = function(packet, index, total) {
    // if its the first message.js we consider the transport open
    ...
    // otherwise bypass onData and handle the message.js
    self.onPacket(packet);
  };
 ...
 * @return {String} base64 encoded message.js
 */
 ...
 *
 * @param {String} base64 encoded message.js
 * @return {Object} with `type` and `data` (if any)
 */
