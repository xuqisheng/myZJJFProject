...
function FormatSpWithDrawPayStatus(value, row){
	var result ;
	if(value == 0){
...
function FormatSpWithDrawStatusX(value, row) {
	var result ;
	if(value == 2){
...
<div class="easyui-layout"  fit="true" id="spWithDrawListMgdgLayout">
	<div region="center">
		<!--=================================================west-table========================================================  -->
		<table id="spWithDrawListMgdg" idField="id"  sortName="applyTime" sortOrder="desc"  pageSize="50" pageList="[50,500,1000,2000]"
			toolbar="#spWithDrawListMgtoolbar" pagination="true" fitColumns="true"  fit="true"
		    ...
		    <thead>
		        ...
		        <th field="wallet">钱包</th>
		        <th field="amount">提现金额</th>
		        <th field="status" formatter="FormatSpWithDrawStatusX">申请状态</th>
		        ...
		        <th field="payState" formatter="FormatSpWithDrawPayStatus">付款状态</th>
		...
		<!--===================================================west-table-toolbar=================================================  -->
		<div id="spWithDrawListMgtoolbar" style="padding:5px;">
			 <div>
		 		 订单状态：<select id="spWithDrawListMgField1" class="easyui-combobox" panelHeight="auto" style="width:70px" >
			  	...
			  	批发商名：<input id="spWithDrawListMgField2" class="easyui-textbox" style="width:140px;">
			  	申请时间：<input id="spWithDrawListMgField3" class="easyui-datebox" style="width:150px">
			  	至：<input id="spWithDrawListMgField4" class="easyui-datebox" style="width:150px">
		   		<a  id="spWithDrawListMgtoolbar-findlist"  class="easyui-linkbutton"  iconCls="icon-search">查询</a>
		   		...
		   		<a  onclick="clearAllSelectSpWithDraw();" class="easyui-linkbutton">清除选择</a>
		    </div>
		</div>
	</div>
	<!-- <div id="spWithDrawListMgLayoutEast" data-options="region:'east',collapsed:true,split:true" title="订单明细" style="width:50%;">
...
<!--===================================================west-table-modeifydlg=================================================  -->
<div id="spWDSheetMgdlg" class="easyui-dialog"  modal="true"  style="min-width:400px;padding:10px 20px;min-height:200px;" closed="true" buttons="#spWDSheetMgdlgbt">
    <div class="ftitle">信息录入</div>
    <form id="spWDSheetMgfm" method="post">
    	<div class="fitem">
...
<div id="spWDSheetMgdlgbt">
	<a id="spWDSheetMgdlgbt-save"  class="easyui-linkbutton c6" iconCls="icon-ok"   style="width:90px">保存</a>
	<a id="spWDSheetMgdlgbt-cancel"   class="easyui-linkbutton" iconCls="icon-cancel"  style="width:90px">取消</a>
</div>
...
$("#spWithDrawListMgField1").val(initStatus);

var mibocrud = MiBoCRUD.creatNew({
	modelName:"spWithDrawListMg",
	dialogTitle:"信息编辑",
	queryParams:{'supplierId':'${supplierId}',status:initStatus},
	listURL:'<c:url value="/account/spwalletctl/DetailList.do"/>',
	creatURL:'<c:url value="/account/spwalletctl/SpAdd.do"/>',
	updateURL:'<c:url value="/account/spwalletctl/SpUpdate.do"/>',
	deleteURL:'<c:url value="/account/spwalletctl/SpDelete.do"/>',
	findListFun:function(){
		$("#spWithDrawListMgdg" ).datagrid('load',{
			supplierId:'${supplierId}',
			status:$("#spWithDrawListMgField1").combobox('getValue'),
			spKeyword:$("#spWithDrawListMgField2").textbox('getValue'),
			beginTime:$("#spWithDrawListMgField3").datebox('getValue')+" 00:00:00",
			endTime:$("#spWithDrawListMgField4").datebox('getValue')+" 23:59:59"
		})
	}
}).init();
...
function doWithDrawPassActionBatch(status){
	var withDrawIds = [];
	var rows = $('#spWithDrawListMgdg').datagrid('getSelections');
...
function goWithDrawSpShengAction(status,withDrawIds){
	$.messager.prompt('审核通过', '请输入备注（选填）:', function(r){
		if (r){
			$.post( '<c:url value="/account/spwalletctl/UpdateWithDrawPass.do"/>',{
				userRemark:r,
				status:status,
				withDrawIds:withDrawIds.join(",")
			},function(result){
				$.messager.alert("信息",""+result.message);
				$('#spWithDrawListMgdg').datagrid('reload');
			},'json');
		}
	});
}
...
function getfeeSpShengSpOrderInfoDetail(spOrderId){
	$('#spWithDrawListMgdgLayout').layout('expand','east');
	$('#spWithDrawListMgLayoutEast').panel('open').panel('refresh',
	root+'/account/spwalletctl/SettleOrderInfoManagerPage.do?spOrderId='+spOrderId);
}
...
$("#toPayWithDrawListMgdlgbt-save").off("click").on("click",function(){
	...
	$("#toPayWithDrawListMgfm").form('submit',{
		url: '<c:url value="/account/spwalletctl/CreatDWSheet.do"/>',
		onSubmit: function(){
		...
		success: function(result){
			...
			if (!result.success){
			    ...
			    $.messager.alert("操作成功",retMessage,"info",function(){
					...
					$("#spWithDrawListMgdg").datagrid('reload'); // reload the user data
			    });
			}
		}
	})
...
function inputSheetReBack(){
	...
	if(rows.length>0){
		...
		$('#spWDSheetMgdlg').dialog('open').dialog('setTitle','录入银行流水,共'+rows.length+'条,金额：'+allmoney+'元');
		$('#spWDSheetMgfm').form('load',rows[0]);
...
function clearAllSelectSpWithDraw(){
	$('#spWithDrawListMgdg').datagrid("clearSelections");
}
//对话框保存按钮
$("#spWDSheetMgdlgbt-cancel").off("click").on("click",function(){
	$('#spWDSheetMgdlg').dialog('close');
});
//对话框保存按钮
var currentWDSheetIds;
$("#spWDSheetMgdlgbt-save").off("click").on("click",function(){
	$("#fmWdSheetIds").val(currentWDSheetIds);
	$("#spWDSheetMgfm").form('submit',{
		url: '<c:url value="/account/spwalletctl/FillWDBack.do"/>',
		onSubmit: function(){
			return $(this).form('validate');
		},
		success: function(result){
