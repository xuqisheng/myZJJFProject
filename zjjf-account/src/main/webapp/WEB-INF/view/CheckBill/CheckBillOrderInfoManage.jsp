...
<div class="easyui-layout"  fit="true">
	<div region="center">
		...
		<div class="wrap s-list noprint">
			...
			<div><span>财务备注：</span><input id="accountRemarkInput" value="${spOrderInfo.acremark}"></div>
			<div>
				<c:choose>
						<c:when test="${spOrderInfo.acstatus==1 }">
							<a id="checkBillPassOrder" onclick="doAccountSpShengAction(2,'${spOrderInfo.orderid}')" class="easyui-linkbutton">审核通过</a>
						</c:when>
						<c:when test="${spOrderInfo.acstatus==2 }">
							<a id="checkBilldePassOrder" onclick="doAccountCancelSpShengAction(1,'${spOrderInfo.orderid}')"  class="easyui-linkbutton">取消审核</a>
						</c:when>
				</c:choose>
			</div>
		</div>
