...
public class SpGroupDailyServiceJobImpl extends AbstractTimeService<Integer, SpGroupDaily> {
	...
	private void appendParam(Integer spGroupId, SpGroupDaily v, String dayTime) {
		...
		if (exitBean != null && exitBean.getSpGroupId() != 0) {
			update_plantdaily(exitBean.getId(), spGroupId, dayTime);
		} else {
			this.add_spGroup_daily(v, spGroupId, dayTime);
		}
	}

	private void add_spGroup_daily(SpGroupDaily v, Integer spGroupId, String dayTime) {
	...
	private void update_plantdaily(Integer spGroupDailyId, Integer spGroupId, String dayTime) {
