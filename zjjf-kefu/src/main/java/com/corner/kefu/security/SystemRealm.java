...
public class SystemRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		if (authcToken == null) {
			...
			try {
				customer = authorityService.getUserByCustomerServiceCredential(credential);
				logger.info("根据手机号获取当前用户：id：{},密码：{}，手机号：{}", customer.getId(), customer.getPassword(), customer.getMobile());
			} catch (Exception e) {
				return null;
			}finally{
				if(customer == null) throw new UnknownAccountException();				
			}
			SimpleAuthenticationInfo sauth = new SimpleAuthenticationInfo(customer.getId(), customer.getPassword(), getName());
			return sauth;
		}
	}
