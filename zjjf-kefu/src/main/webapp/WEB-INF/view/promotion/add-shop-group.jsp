...
<body class="wrap-bd">
		...
		<form action="" id="promotionForm">
			...
			<div class="mb-default clearfix">
				...
				<div class="fl" style="width:870px">
					...
					<c:forEach var="shopGroup" items="${list}">
						<span style="display: inline-block;width:285px;height:20px;overflow:hidden">
							<input type="checkbox" value="${shopGroup.id}"  name="spGroupId" class="spGroupCheckBox" id="${shopGroup.id}" /> ${shopGroup.name}
						</span>
                    </c:forEach>
                   </div>
			</div>
	...
	<script>
		...
		function checkForm() {
			$.ajax({
    			type : "POST",
    			url : "${root}/Customer/proManage/saveCuxiao.do",
    			async : false,
