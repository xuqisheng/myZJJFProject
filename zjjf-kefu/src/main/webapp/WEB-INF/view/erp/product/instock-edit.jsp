...
<div class="wrap-bd">
        ...
        <form id="detailFrom">
                ...
                <table class="table-list mt-small">
                    ...
                    <c:forEach items="${info.details}" var="ele" varStatus="eleStat">
                        <tr>
                            ...
                            <%--<th>
                                <input type="text" name="producingAreas" maxlength="20" value="" class="input-search-date" <c:if test="${ele.quantity == ele.operateQuantity}">disabled="disabled" </c:if>>
                            </th>--%>
                        </tr>
                    </c:forEach>
                    </tbody>
            ...
            <div class="wrap-bd bg">
                <p>
                    ...
                    <textarea name="remark" class="textarea" maxlength="200"></textarea>
                </p>
                <div>
                    <label class="label"></label>
                    <input type="button" name="" value="确认入库" class="button-save">
    ...
    <c:forEach items="${lists}" var="ele">
        <div class="wrap-bd bg mt-small">
            <div class="clearfix">
                <c:forEach items="${ele}" var="ele2" varStatus="loopstatus">
                    <c:if test="${loopstatus.first}">
                        <div class="fl mb-default">
                            入库单号：${ele2.erpPlantItemLog.orderId}
                            <c:set var="orderId2" value="${ele2.erpPlantItemLog.orderId}"/>
                            <span class="ml-default">入库时间：<fmt:formatDate value="${ele2.erpPlantItemLog.addTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
                            <span class="ml-default">入库人：${ele2.erpPlantItemLog.actionUserName}</span>
                            <br><br>
                            备注：${ele2.erpPlantItemLog.remark}
                        </div>
                    </c:if>
                </c:forEach>
                <a href="${root}/scms/ERPOrderInfo/detail/${info.orderId}/${orderId2}.do" target="_blank" class="fr button">打印入库单</a>
            </div>
            <table class="table-list table-border">
                ...
                <c:forEach items="${ele}" var="ele2">
                    <tr>
                        ...
                        <td>${ele2.spec}</td>
                        <th>${ele2.quantity}</th>
                        <th>${ele2.erpPlantItemLog.operateQuantity}</th>
                        <th>${ele2.erpPlantItemLog.goodsStock}</th>
                        <th><fmt:formatDate value="${ele2.erpPlantItemLog.productionDate}" pattern="yyyy-MM-dd"/></th>
                        <th>${ele2.erpPlantItemLog.producingArea}</th>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
    ...
    $(function () {
        ...
        $('.button-save').on('click' , function () {
            ...
            $('#detailFrom').find('input[name="quantitys"]').each(function(){
                ...
                if((Number(quantitys) == 0 || quantitys == null || quantitys == '') && (productionDates == null || productionDates == '')){
                    return true
