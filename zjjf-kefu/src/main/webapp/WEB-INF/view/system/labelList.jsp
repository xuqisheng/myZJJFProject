...
<div class="mb-small clearfix">
    ...
    <div class="fr">
        <a href="${root }/kefu/appItemLabel/returnEditPage/1.do" class="button button-default">新增标贴</a>
    </div>
</div>
<div id="abc">
...
$(function() {
	...
	$("#jpagination").pagination({
	    pageSize: 10,
	    remote: {
	        url: '${root}/kefu/appItemLabel/getAllAppItemLabel.do',
	        ...
	        success: function(data) {
	            ...
	            $.each(data.list, function(i,item) {
	            	...
	            	html+='<td><img style="width:100px;height:40px; alt="" src="${USER_FASTFDSPREURL}'+item.tagImg+'" class="imgs"></td>';
	            	if(item.status==0){
	            		html+='<td>停用</td>';
	            	}else{
	            		html+='<td>正常</td>';
	            	...
	            	if(item.isDelete){
	            		...
	            		html+='<a href="${root}/kefu/appItemLabel/returnEditPage/2.do?id='+item.id+'" class="button button-operate">编辑</a>';
	            	}
	            	html+='</td>';
                    html+='</tr>';
	            });
	...
	$('#abc').on('click','.staBtn',function(){
		...
		console.log(status);
		$.ajax({
			url:'${root}/kefu/appItemLabel/updateStopOrStart/'+id+'/'+status+'.do',
			type:'post',
			dataType:'json',
			success:function(data){
				if(data.success){
	...
	$('#abc').on('click','.delBtn',function(){
		...
		$.ajax({
			url:'${root}/kefu/appItemLabel/updateDelOrReco/'+id+'/'+isdelete+'.do',
			type:'post',
			dataType:'json',
			success:function(data){
				if(data.success){
