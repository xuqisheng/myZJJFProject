...
<body class="wrap-bd" ng-controller="tableController" data-ng-init="loadPage()">
	<div class="mb-default">
	    ...
	    <a href="" class="crumb crumb-active">商品明细报表</a>
	</div>
	...
	<div class="op-section">
	    <input type="text" placeholder="订单编号" class="input input-search-text" ng-model="orderNos">
	...
	<div class="op-section" style="border-left: 3px solid #00a0e9;">
	           订单总额:&nbsp;&nbsp;<span style="font-size: 32px; color: #000">{{orderMummary.orderPriceSum}}</span>&nbsp;
	</div>
	        ...
	        <tbody class="table-tbody">
	       		<tr ng-repeat="d_row in key_dataList">
		       		<td ng-repeat="b in d_row track by $index">
		       			<a ng-if="$index == '7'" ng-href="<%=request.getContextPath() %>/api/sc_order/detailLoadPage.do?orderId={{d_row[5]}}">
		       				{{b}}
		       			</a>
		       			<span ng-if="$index != '7'">
		       				{{b}}
	...
	<script>
	    ...
	    var export_url = root + '/api/sc_order/portExcel.do';
	    var changeCity_url = root + '/api/base_core/getCityAreaGroupById.do';
	    var getQueryParam_url = root + '/api/sc_order/getQueryParam.do';
	    ...
	    app.controller('tableController', ['$scope', 'spOrderSevrices', function ($scope, spOrderSevrices) {
			...
			$scope.queryOrders = function(flag){
				...
				spOrderSevrices.getOrderList(data).success(function (result) {
		    		...
		    		$scope.orderMummary = result.orderMummary;
