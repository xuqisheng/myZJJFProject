...
<div class="wrap-bd">
	<div class="mb-small">
	    ...
	    <a href="${root}/scms/wh/toWhWalletIndex.do" class="crumb">我的钱包</a><span class="crumb crumb-active">收支明细</span>
	</div>
	<div class="op-section">
		<div class="title mb-default">收支明细</div>
		<form action="${root}/scms/wh/toWhWalletLog.do" name="frm_query" method="get" id="incomForm">
			<label>查询日期</label>
		...
		<table class="table-list">
			...
			<tbody class="table-tbody">
				<c:forEach items="${list}" var="log">
					<tr><td><span class="color-gray"><fmt:formatDate value="${log.actionTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span></td>
					...
					<td>
						<c:choose>
							<c:when test="${log.optType == 0 && log.actionType==1}">默认收入</c:when>
								...
								<c:when test="${log.optType == 11 && log.actionType==1}">转账入账</c:when>
								...
								<c:when test="${log.optType == 11 && log.actionType==2}">转账出账</c:when>
								<c:otherwise>其他</c:otherwise>
						</c:choose>
						:
						<span class="color-gray">
							<c:choose>
								<c:when test="${log.optType == 4 || log.optType == 9}">${log.whWithDrawId }</c:when>
					...
					<td>
						<c:choose>
							...
							<c:when test="${log.optType == 11}">对公转账</c:when>
							<c:otherwise>转角账户</c:otherwise>
						</c:choose>
					<td>
						<c:choose>
							<c:when test="${log.optType == 4 || log.optType == 9}"><a href="${root}/scms/wh/toWhWithDrawById.do?id=${log.orderId}">查看</a></c:when>
							<c:otherwise>
								<c:if test="${log.orderId != null && log.orderId != ''}"><a href="${root}/scms/scOrder/getWarehouseOrderList.do?orderIdAndSupply=${log.orderId}">查看</a></c:if>
							</c:otherwise>
						</c:choose>
					</td>
					</tr>
