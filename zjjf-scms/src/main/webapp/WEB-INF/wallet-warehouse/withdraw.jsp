...
<div class="mb-small">
    ...
    <a href="${root}/scms/wh/toWhWalletIndex.do" class="crumb">我的钱包</a><span class="crumb crumb-active">提现</span>
</div>
<div class="bg">
			...
			<tbody>
				<tr>
					...
					<td colspan="2" class="td1">
                        <div class="info">
                            <span>户名：${scmsWarehouse.bankUserName}</span><br />
                            <span>${scmsWarehouse.bankName}  卡号：${scmsWarehouse.bankNum}</span>
                        </div>
					</td>
				</tr>
				<tr>
...
<script>
	...
	$(function() {
		...
		$('#btn_next').on('click', function(){
			if('${scmsWarehouse.bankUserName}' == '' || '${scmsWarehouse.bankName}' == '' || '${scmsWarehouse.bankNum}' == ''){
				...
				if(flag) {
					$.ajax({
						type: "GET",
						url: "${root}/scms/wh/doWithDraw.do",
						dataType: "json",
						data: {presentMoney:$.trim($('#withdrawMoney').val())},
						success: function(data) {
							if(data.success) {
								layer.msg(data.message.msg);
								location.href = "${root}/scms/wh/toWhWithDrawById.do?id=" + data.message.status;
							} else {
								layer.msg(data.message.msg);
							} 
						},
