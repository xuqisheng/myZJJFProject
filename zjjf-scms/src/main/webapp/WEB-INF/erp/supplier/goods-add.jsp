...
<div class="wrap-bd">
    <div class="mb-default">
        ...
        <a class="crumb" href="${root }/scms/ERPMaItem/getAllManagerItem.do?managerId=${manager.id }">商品管理<span id="J_bbbbb">（${manager.managerName }）</span></a>
    ...
    <div class="table-contain">
        <pre>
            <table class="table-list">
                ...
                <tr>
                    <td colspan="8">
                        <span class="button-operate addTr">添加</span>
                    </td>
                </tr>
                </tbody>
            </table>
    ...
    <div class="ta-c mt-default">
        <input type="button" value="确定" class="button-save" id="okSave">
	...
	$(function(){
		...
		$("#tb").on("change",".mdseId",function(){
			...
			if(mdseId != null && mdseId != "" && mdseId != "null" && mdseId != "undefined"){
				$.post("${root}/scms/ERPMaItem/getitemByMdseId.do?action=1",{mdseId:mdseId},function(data){
					if(data.success){
		...
		$("#tb").on("change",".mdseIdx",function(){
			...
			var mdseId = $root.val();
			if(mdseId != null && mdseId != "" && mdseId != "null" && mdseId != "undefined"){
				$.post("${root}/scms/ERPMaItem/getitemByMdseId.do?action=2",{mdseId:mdseId},function(data){
					if(data.success){
		...
		$("#okSave").on("click",function(){
			...
			$.post("${root}/scms/ERPMaItem/addERPManagerItem.do",$("#param").serializeArray(),function(data){
				if(data.success){
					alert(data.message);
					location.href="${root }/scms/ERPMaItem/getAllManagerItem.do?managerId="+managerId;
				}else{
					alert(data.message);
				}
		...
		$("#okClose").on("click",function(){
			location.href="${root }/scms/ERPMaItem/getAllManagerItem.do?managerId=${manager.id }";
		});
	});
	//判断商品是否重复
