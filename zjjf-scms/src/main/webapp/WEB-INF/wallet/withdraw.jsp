...
<div class="mb-small">
    ...
    <a href="${root}/scms/sp/toSpWalletIndex.do" class="crumb">我的钱包</a><span class="crumb crumb-active">提现</span>
</div>
<div class="bg">
	<div class="wallet-withdraw-title">
		<h2>提取余额到银行卡</h2>
		余额:&nbsp;<span class="yue">${finWallet.balance}</span>&nbsp;元
	</div>
			...
			<tbody>
				<tr>
					...
					<td colspan="2" class="td1">
                        <div class="info">
                            <span>户名：${supplier.cardUser}</span><br />
                            <span>${supplier.bankName}  卡号：${supplier.bankNum}</span>
                        </div>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan="2" class="td2">
                       ...
                       <span class="color-gray"><c:if test="${finWallet.rates > 0}">（扣除${rates}的手续费）</c:if><c:if test="finWallet.rates <= 0">（免手续费）</c:if></span>
					</td>
				</tr>
				<tr>
					<td>到账时间：</td>
...
<script>
	...
	$(function() {
		$('#withdrawMoney').keyup(function() {
			...
			$.ajax({
				type: "POST",
				url: "${root}/scms/sp/caculateFee.do",
				dataType: "json",
				data: {withdrawMoney:$.trim($(this).val())},
				success: function(data) {
					if(data.success) {
						$('#presentMoney').text(data.message.daozhanjine);
					} else {
		...
		$('#btn_next').on('click', function(){
			...
			if(flag) {
				$.ajax({
					type: "GET",
					url: "${root}/scms/sp/doWithDraw.do",
					dataType: "json",
					data: {presentMoney:$.trim($('#withdrawMoney').val())},
					success: function(data) {
						if(data.success) {
							layer.msg(data.message.msg);
							location.href = "${root}/scms/sp/toSpWithDrawDealLog.do?withDrawId=" + data.message.status;
						} else {
							layer.msg(data.message.msg);
						}
					},
