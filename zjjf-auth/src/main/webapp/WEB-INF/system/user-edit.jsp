...
<div class="mt-small mb-default">
    <a href="${root }/auth/user/getUserListPage.do">返回系统用户列表</a>
...
<form id = "addEditParam">
	<fieldset>
	   ...
	   <p class="required">
	       ...
	       <input class="input input-default" type="text" name="nickName" id="nickName" value="${user.nickName }" maxlength="16">
	   </p>
	   <p>
	       <label class="label">手机号：</label>
	       <input class="input input-default" type="text" name="mobile" id="mobile" value="${user.mobile }" maxlength="15" >
	   	   <input type="hidden" id="mobile1" value="${user.mobile }">
	   </p>
	   <p>
	       ...
	       <c:choose>
	       	<c:when test="${user.gender==1 }">
		        ...
		        <input class="ml-default" type="radio" name="gender" value="0"> 女
	       	</c:when>
	       	<c:when test="${user.gender==1 }">
	       		<input type="radio" name="gender" value="1" > 男
		        <input class="ml-default" type="radio" name="gender" value="0" checked="checked"> 女
	       	</c:when>
	       	<c:otherwise>
		       	<input type="radio" name="gender" value="1" checked="checked"> 男
			    <input class="ml-default" type="radio" name="gender" value="0" > 女
			...
			if(id == '' || id == null){
			 var password = $.trim($('input[name="password"]').val());
		 		if(password == ''){
					...
					$.ajax({
		     			type : "POST",
		     			url : "${root }/auth/user/insertUser.do",
		     			dataType : "json",
				...
				$.ajax({
	     			type : "POST",
	     			url : "${root }/auth/user/updateUser.do",
	     			dataType : "json",
	     			data:$('#addEditParam').serialize(),
	     			success : function(data) {
	     				if (data.success) {
