...
<div class="mt-small mb-default">
    <a href="${root }/auth/admRole/getRoleListPage.do">返回角色列表</a>
</div>
...
<script>
	$(function() {
		...
		$('#roleNo').on('blur' , function(){
			...
			if(val == ''){
				...
				$.post("${root}/auth/admRole/checkRoleNo.do",{roleNo:val},function(data){
					if(data.success){
						alert(data.message);
						$("#roleNo").val("");
						$("#roleNo").focus();
	...
	function saveInfo(){
		...
		if(id == '' || id == null){
			$.ajax({
     			type : "POST",
     			url : "${root }/auth/admRole/insertRole.do",
     			dataType : "json",
     			data:$('#addEditParam').serialize(),
     			success : function(data) {
     				if (data.success) {
	   					alert(data.message);
	   					location.href="${root }/auth/admRole/getRoleListPage.do";
	   				}else{
	   					alert(data.message);
	   				}
     			},
			...
			$.ajax({
     			type : "POST",
     			url : "${root }/auth/admRole/updateRole.do?id="+id,
     			dataType : "json",
     			data:$('#addEditParam').serialize(),
     			success : function(data) {
     				if (data.success) {
